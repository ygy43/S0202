            ...
            Select Case selectedData.Series.series_kataban.Trim
                    ...
                    Select Case selectedData.Series.key_kataban.Trim
                            ...
                            If selectedData.Symbols(4).Trim <> "" Then
                                Select Case selectedData.Symbols(1).Trim
                                    Case "4", "5"
                                        ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                                        ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                                        strOpRefKataban(UBound(strOpRefKataban)) = "AVB41/51" & MyControlChars.Hyphen & _
                                    ...
                                    Case "6", "7"
                                        ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                                        ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                                        strOpRefKataban(UBound(strOpRefKataban)) = "AVB61/71" & MyControlChars.Hyphen & _
                                                                                   selectedData.Symbols(4).Trim & MyControlChars.Hyphen & _
                                                                                   selectedData.Symbols(6).Trim
                                        decOpAmount(UBound(decOpAmount)) = 1
                                End Select
                        ...
                        Case "2"
                            ...
                            If selectedData.Symbols(5).Trim <> "" Then
                                ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                                ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                                strOpRefKataban(UBound(strOpRefKataban)) = "AVB5/6/7/8" & MyControlChars.Hyphen & _
                        ...
                        Case "3"
                            ...
                            'コイル加算価格キー
                            If selectedData.Symbols(5).Trim = "2G" Then
                                ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                                ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                                strOpRefKataban(UBound(strOpRefKataban)) = Left(selectedData.Series.series_kataban.Trim, 3) & MyControlChars.Hyphen & _
                                                                           selectedData.Symbols(5).Trim
                        ...
                        Case "4"
                            ...
                            If selectedData.Symbols(4).Trim.Length <> 0 Then
                                ...
                                'H0M選択字、高温用マグネット加算価格
                                If selectedData.Symbols(4).Trim.Equals("H0M") Then
                                    ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                                    ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                                    strOpRefKataban(UBound(strOpRefKataban)) = "AVB**7" & MyControlChars.Hyphen & _
                                                                               selectedData.Symbols(3).Trim & MyControlChars.Hyphen & _
                                                                               selectedData.Symbols(4).Trim
                                    decOpAmount(UBound(decOpAmount)) = 1
                            ...
                            If selectedData.Symbols(7).Trim <> "" Then
                                ...
                                ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                                '2011/02/14 MOD RM1102033(3月VerUP:AVBシリーズ　引当項目追加) START--->
                                strOpRefKataban(UBound(strOpRefKataban)) = "AVB*17" & MyControlChars.Hyphen & _
                        ...
                        Case "6" '2008/10/7 AVB*47
                            ...
                            If selectedData.Symbols(5).Trim.Length <> 0 Then
                                ...
                                'H0M選択字、高温用マグネット加算価格
                                If selectedData.Symbols(5).Trim.Equals("H0M") Then
                                    ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                                    ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                                    strOpRefKataban(UBound(strOpRefKataban)) = "AVB**7" & MyControlChars.Hyphen & _
                                                                               selectedData.Symbols(4).Trim & MyControlChars.Hyphen & _
                        ...
                        Case "7"
                            ...
                            If selectedData.Symbols(6).Trim <> "" Then
                                ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                                ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                                strOpRefKataban(UBound(strOpRefKataban)) = "AVB*37" & MyControlChars.Hyphen & _
                                                                           selectedData.Symbols(7).Trim & MyControlChars.Hyphen & _
                                                                           selectedData.Symbols(9).Trim
                                decOpAmount(UBound(decOpAmount)) = 1
                ...
                Case "AVP"
                    ...
                    If selectedData.Symbols(6).Trim <> "" Then
                        ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                        ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                        strOpRefKataban(UBound(strOpRefKataban)) = "AVP5/6/7/8" & MyControlChars.Hyphen & _
                                                                   selectedData.Symbols(6).Trim & MyControlChars.Hyphen & _
                                                                   selectedData.Symbols(8).Trim
                        decOpAmount(UBound(decOpAmount)) = 1
                ...
                Case "EVB"
                    ...
                    strOpRefKataban(UBound(strOpRefKataban)) = selectedData.Series.series_kataban.Trim & _
                                                               ...
                                                               selectedData.Symbols(4).Trim & MyControlChars.Hyphen & _
                                                               selectedData.Symbols(5).Trim & MyControlChars.Hyphen & _
                                                               "03" & MyControlChars.Hyphen & selectedData.Symbols(7).Trim
                    decOpAmount(UBound(decOpAmount)) = 1

                    'ケーブルオプション加算価格キー(3m以内)
                    If selectedData.Symbols(6).Trim <> "03" Then
                        ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                        ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
