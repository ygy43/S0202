            ...
            Select Case selectedData.Series.series_kataban
                Case "BHA"
                    If selectedData.Series.key_kataban.Trim = "P" Then
                        strOptionFP1 = selectedData.Symbols(7)
                    Else
                        If selectedData.Symbols.Count >= 8 Then
                            strOptionP4 = selectedData.Symbols(7)
                        End If
                    End If
                Case "HMF"
                    ...
                    End If
                Case "CKL2"
                    If selectedData.Series.key_kataban.Trim = "5" Then
                        strOptionFP1 = selectedData.Symbols(7)
                    Else
                        If selectedData.Symbols.Count >= 8 Then
                            strOptionP4 = selectedData.Symbols(7)
                ...
                Case Else
                    ...
                    If selectedData.Symbols(3).Trim <> "" Then
                        ...
                        strOpRefKataban(UBound(strOpRefKataban)) = selectedData.Series.series_kataban.Trim & MyControlChars.Hyphen & _
                                                                   ...
                                                                   selectedData.Symbols(1).Trim
                        If Left(selectedData.Series.series_kataban.Trim, 1) = "C" Then
                            ...
                            Case "CKG"
                                Select Case selectedData.Symbols(1).Trim
                                    Case "50CS"
                                        ...
                                        ReDim Preserve strPriceDiv(UBound(strPriceDiv) + 1)     'RM0908030 2009/10/28 Y.Miura
                                        strOpRefKataban(UBound(strOpRefKataban)) = selectedData.Series.series_kataban.Trim & MyControlChars.Hyphen & _
                                                                                   selectedData.Symbols(4).Trim & MyControlChars.Hyphen & "T"
                                        decOpAmount(UBound(decOpAmount)) = KatabanUtility.SwitchQtyGet(selectedData.Symbols(6).Trim)
                                        'RM0908030 2009/10/28 Y.Miura
                                        If bolC5Flag = True Then
                                            strPriceDiv(UBound(strPriceDiv)) = AccumulatePriceDiv.C5
                    ...
                    If strOptionFP1 <> "" Then
                        Select Case selectedData.Symbols(2)
                            Case ""
                                ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                                ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                                ReDim Preserve strPriceDiv(UBound(strPriceDiv) + 1)
                        ...
                        Select Case selectedData.Series.series_kataban.Trim
                            Case "BHE"
                                If selectedData.Symbols(2).Trim <> "" Then
                                    strOpRefKataban(UBound(strOpRefKataban)) = selectedData.Series.series_kataban.Trim & MyControlChars.Hyphen & _
                                                                               selectedData.Symbols(1).Trim & MyControlChars.Hyphen & _
                                                                               selectedData.Symbols(2).Trim & "-OP-" & strOptionP4
                                Else
                                    strOpRefKataban(UBound(strOpRefKataban)) = selectedData.Series.series_kataban.Trim & MyControlChars.Hyphen & _
                                                                               selectedData.Symbols(1).Trim & "-OP-" & strOptionP4
                                End If
                            Case "BHA"              'RM1001045 2010/02/23 Y.Miura 追加
                                strOpRefKataban(UBound(strOpRefKataban)) = selectedData.Series.series_kataban.Trim & "-OP-" & strOptionP4
                            Case "HMF"
                                strOpRefKataban(UBound(strOpRefKataban)) = selectedData.Series.series_kataban.Trim & MyControlChars.Hyphen & _
                                                                          selectedData.Symbols(1).Trim & "-OP-" & strOptionP4
                            Case Else
                                strOpRefKataban(UBound(strOpRefKataban)) = selectedData.Series.series_kataban.Trim & MyControlChars.Hyphen & _
                                                                         strOptionP4 & MyControlChars.Hyphen & selectedData.Symbols(1).Trim
                        End Select
                        decOpAmount(UBound(decOpAmount)) = 1
                    End If
            End Select
