...
Module KHPriceK5
    ...
    Public Sub subPriceCalculation(selectedData As SelectedInfo, _
        ...
        Try
                ...
                Select Case selectedData.Symbols(7).Trim
                    ...
                    Case Else
                        ...
                        If selectedData.Symbols(7).Trim <> "" Then
                            If selectedData.Series.series_kataban.IndexOf("L2") >= 0 Then
                                ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                                ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                                strOpRefKataban(UBound(strOpRefKataban)) = Left(selectedData.Series.series_kataban.Trim, 3) & _
                                                                           selectedData.Symbols(7).Trim & "-L2-" & _
                                                                           selectedData.Symbols(6).Trim
                                decOpAmount(UBound(decOpAmount)) = KatabanUtility.SwitchQtyGet(selectedData.Symbols(8).Trim)
                            Else
            ...
            'G1(コイルスクレーパ)加算価格キー
            If selectedData.Series.series_kataban.IndexOf("G1") >= 0 Then
                ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                strOpRefKataban(UBound(strOpRefKataban)) = Left(selectedData.Series.series_kataban.Trim, 3) & "-G1-" & _
                                                           selectedData.Symbols(2).Trim
                decOpAmount(UBound(decOpAmount)) = 1
            End If
