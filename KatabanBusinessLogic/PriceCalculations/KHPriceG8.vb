        ...
        Try
            ...
            Select Case selectedData.Symbols(1).Trim
                Case "00-0"
                    ...
                    ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                    ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                    strOpRefKataban(UBound(strOpRefKataban)) = "KML60-" & selectedData.Symbols(2).Trim & "REN"
                    decOpAmount(UBound(decOpAmount)) = 1
                Case Else
                    '1点式KML50混合搭載基本価格キー
                    ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                    ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                    strOpRefKataban(UBound(strOpRefKataban)) = selectedData.Symbols(2).Trim & "REN-" & _
                                                               intKML50Qty.ToString & "MIX-KML50-" & _
                                                               selectedData.Symbols(1).Trim & "-0"
                    decOpAmount(UBound(decOpAmount)) = 1
