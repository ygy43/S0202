            ...
            If selectedData.Symbols(1).Trim <> "" Then
                Select Case selectedData.Symbols(1).Trim
                    Case "DQ", "DC", "DZ", "DH", "DT", _
                        ...
                        ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                        ReDim Preserve strPriceDiv(UBound(strPriceDiv) + 1)
                        strOpRefKataban(UBound(strOpRefKataban)) = Left(selectedData.Series.series_kataban.Trim, 4) & "-VAR-" & _
                                                                   Mid(selectedData.Symbols(1).Trim, 2, 2) & MyControlChars.Hyphen & _
                    ...
                    Case "DM"
                        Select Case True
                            Case CInt(selectedData.Symbols(5).Trim) <= 300
                                ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                                ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                                ReDim Preserve strPriceDiv(UBound(strPriceDiv) + 1)
