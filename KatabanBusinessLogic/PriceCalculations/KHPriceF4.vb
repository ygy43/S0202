...
Module KHPriceF4
    ...
    Public Sub subPriceCalculation(selectedData As SelectedInfo, _
        ...
        Try
            ...
            'Z軸ストロークの値を求める(中間STまるめ処理)
            Select Case True
                Case CInt(selectedData.Symbols(5).Trim) <= 30
                    intZStroke = 30
                Case 31 <= CInt(selectedData.Symbols(5).Trim) And CInt(selectedData.Symbols(5).Trim) <= 50
                    intZStroke = 50
                Case 51 <= CInt(selectedData.Symbols(5).Trim) And CInt(selectedData.Symbols(5).Trim) <= 70  '51～70mm
                    intZStroke = 70
                Case 71 <= CInt(selectedData.Symbols(5).Trim)
                    intZStroke = 100
            End Select
            ...
            Select Case selectedData.Symbols(1).Trim
                Case "1"
                    ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                    ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                    strOpRefKataban(UBound(strOpRefKataban)) = "NHS-L-" & strKSymbol & MyControlChars.Hyphen & intZStroke.ToString
                    decOpAmount(UBound(decOpAmount)) = 1
                Case "2"
                    ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                    ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                ...
                Case "A"
                    Select Case selectedData.Symbols(1).Trim
                        Case "1"
                            ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                            ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                            strOpRefKataban(UBound(strOpRefKataban)) = "NHS-L-" & selectedData.Symbols(10).Trim
                            decOpAmount(UBound(decOpAmount)) = 1
                        Case "2"
                            ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                            ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                ...
                Case "Q"
                    Select Case selectedData.Symbols(1).Trim
                        Case "1"
                            ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                            ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                            strOpRefKataban(UBound(strOpRefKataban)) = "NHS-L-" & selectedData.Symbols(11).Trim & MyControlChars.Hyphen & strKSymbol
                            decOpAmount(UBound(decOpAmount)) = 1
                        Case "2"
                            ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                            ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
