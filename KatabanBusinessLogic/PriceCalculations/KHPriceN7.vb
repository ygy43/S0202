...
Module KHPriceN7
    ...
    Public Sub subPriceCalculation(selectedData As SelectedInfo, _
        ...
        Try
                ...
                Case "M7"
                    Select Case selectedData.Symbols(1).Trim
                        Case "1"
                            ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                            ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                            strOpRefKataban(UBound(strOpRefKataban)) = "W4G4" & MyControlChars.Hyphen & selectedData.Symbols(6).Trim & MyControlChars.Hyphen & "S"
                            decOpAmount(UBound(decOpAmount)) = 1
                        Case "2", "3", "4", "5"
                            ReDim Preserve strOpRefKataban(UBound(strOpRefKataban) + 1)
                            ReDim Preserve decOpAmount(UBound(decOpAmount) + 1)
                            strOpRefKataban(UBound(strOpRefKataban)) = "W4G4" & MyControlChars.Hyphen & selectedData.Symbols(6).Trim & MyControlChars.Hyphen & "D"
                            decOpAmount(UBound(decOpAmount)) = 1
                    End Select
            End Select
            ...
            '電圧加算(AC110Vは加算する)
            If selectedData.Symbols(8).Trim = "5" Then
